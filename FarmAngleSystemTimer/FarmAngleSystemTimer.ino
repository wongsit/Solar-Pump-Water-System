...
void Timer1(int Tnow,int Tminute, int Ton, int Toff);
void pump_on(void);
void pump_off(void);
...
void loop () {
  ...
  if (control_mode == 1) { //Auto
    ...
    if (now.hour() != 165) {            /* Check RTC Operating if 165 = error */      
      ...
      if ((now.second() % 10) == 0) {
        Timer1(now.hour(),now.minute(), 8, 14);          /* call Timer1 Control Operation Hour Start, Stop*/
...
void Timer1(int Tnow,int Tminute, int Ton, int Toff) {
  if ((Tnow >= Ton) && (Tnow <= Toff)) {
    ...
    //water_level = check_water();  /* Check water level */
    if (Tnow < 13){  //morning to midday high solar power
     if(Tminute>30 && Tminute<=40){   
        valve1_off();  
        valve2_on();     
    }else if(Tminute>40){
        valve1_on();
    ...
    }else{    //Afternoon low solar power
      if(Tminute>35 && Tminute<=40){   
        valve1_off();  
        valve2_on();        
    }else if(Tminute>40){
        valve1_on();
